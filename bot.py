...
class AdminStates(StatesGroup):
    ...
    # Состояния для бесплатных товаров
...
class Database:
    ...
    def process_referral_bonus(self, purchase_id: int):
        with self.get_connection() as conn:
            ...
            bonus_amount = purchase['amount_rub'] * 10 / 100
...
# ==================== Обработчики пользовательских функций ====================
...
async def manage_emojis(message: Message, state: FSMContext):
    ...
    settings_emoji = db.get_emoji(EmojiType.SETTINGS.value)
    await message.answer(
        f"{settings_emoji} Управление эмодзи\n\nВыберите эмодзи для изменения:",
...
async def edit_emoji(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        f"2️⃣ Премиум эмодзи в формате: ⭐|123456789"
    )
