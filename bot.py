...
class SetupStates(StatesGroup):
    ...
    waiting_for_refund_policy = State()
    waiting_for_min_order_amount = State()
    waiting_for_max_order_amount = State()
    waiting_for_discount_threshold = State()
...
class AdminStates(StatesGroup):
    ...
    # Раздельные состояния для платных и бесплатных товаров
    ...
    waiting_for_paid_product_photo = State()
    ...
    waiting_for_free_product_photo = State()
...
def get_main_keyboard(telegram_id: int) -> ReplyKeyboardMarkup:
    ...
    # Четвертая строка (информационные кнопки)
    builder.row(
...
def get_admin_panel_keyboard() -> ReplyKeyboardMarkup:
    ...
    # Раздельные кнопки для добавления товаров
    builder.row(
...
@dp.message(lambda message: message.text and "Расширенные настройки" in message.text)
async def advanced_settings(message: Message, state: FSMContext):
    if not db.is_admin(message.from_user.id):
        error_emoji = db.get_emoji(EmojiType.ERROR.value)
    ...
    await message.answer(
        "⚙ Расширенные настройки бота:\n\n"
        "Настройте дополнительные параметры магазина:",
...
async def setup_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if action == "name":
    ...
    elif action == "bot_description":
        await callback.message.edit_text(
            "📝 Введите описание бота:",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_bot_description)
        ...
        await state.set_state(SetupStates.waiting_for_contact_info)
    
    elif action == "rules":
        await callback.message.edit_text(
            "📋 Введите правила магазина:",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_rules)
    
    elif action == "delivery_info":
        await callback.message.edit_text(
            "🚚 Введите информацию о доставке:",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_delivery_info)
    
    elif action == "payment_methods":
        await callback.message.edit_text(
            "💳 Введите способы оплаты:",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_payment_methods)
    
    elif action == "refund_policy":
        await callback.message.edit_text(
            "↩️ Введите политику возврата:",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_refund_policy)
    
    elif action == "min_order":
        await callback.message.edit_text(
            "💰 Введите минимальную сумму заказа в рублях (0 для отключения):",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_min_order_amount)
    
    elif action == "max_order":
        await callback.message.edit_text(
            "💰 Введите максимальную сумму заказа в рублях:",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_max_order_amount)
    
    elif action == "discount":
        await callback.message.edit_text(
            "Введите порог суммы для скидки и процент через пробел (например: 5000 5):",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_discount_threshold)
    
    elif action == "bonus_percent":
        await callback.message.edit_text(
            "👥 Введите процент реферального бонуса (число от 1 до 100):",
            reply_markup=get_back_keyboard()
        )
        await state.set_state(SetupStates.waiting_for_bonus_percent)
...
@dp.message(SetupStates.waiting_for_bot_description)
async def process_bot_description(message: Message, state: FSMContext):
    ...
    db.update_settings(bot_description=message.text)
    
    await state.clear()
    await message.answer(
        f"{success_emoji} Описание бота сохранено!",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(SetupStates.waiting_for_contact_info)
async def process_contact_info(message: Message, state: FSMContext):
    ...
    db.update_settings(contact_info=message.text)
    
    await state.clear()
    await message.answer(
        f"{success_emoji} Контактная информация сохранена!",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(SetupStates.waiting_for_rules)
async def process_rules(message: Message, state: FSMContext):
    ...
    db.update_settings(rules=message.text)
    
    await state.clear()
    await message.answer(
        f"{success_emoji} Правила сохранены!",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(SetupStates.waiting_for_delivery_info)
async def process_delivery_info(message: Message, state: FSMContext):
    ...
    db.update_settings(delivery_info=message.text)
    
    await state.clear()
    await message.answer(
        f"{success_emoji} Информация о доставке сохранена!",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(SetupStates.waiting_for_payment_methods)
async def process_payment_methods(message: Message, state: FSMContext):
    ...
    db.update_settings(payment_methods=message.text)
    
    await state.clear()
    await message.answer(
        f"{success_emoji} Способы оплаты сохранены!",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(SetupStates.waiting_for_refund_policy)
async def process_refund_policy(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    success_emoji = db.get_emoji(EmojiType.SUCCESS.value)
    ...
    db.update_settings(refund_policy=message.text)
    
    await state.clear()
    await message.answer(
        f"{success_emoji} Политика возврата сохранена!",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(SetupStates.waiting_for_min_order_amount)
async def process_min_order(message: Message, state: FSMContext):
    ...
    try:
        ...
        db.update_settings(min_order_amount=amount)
        
        await state.clear()
        await message.answer(
            f"{success_emoji} Минимальная сумма заказа установлена: {amount} ₽",
            reply_markup=get_main_keyboard(message.from_user.id)
        )
    except ValueError:
        await message.answer(
            f"{error_emoji} Введите корректное число:",
            reply_markup=get_cancel_keyboard()
        )

@dp.message(SetupStates.waiting_for_max_order_amount)
async def process_max_order(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    success_emoji = db.get_emoji(EmojiType.SUCCESS.value)
    ...
    try:
        ...
        db.update_settings(max_order_amount=amount)
        
        await state.clear()
        await message.answer(
            f"{success_emoji} Максимальная сумма заказа установлена: {amount} ₽",
            reply_markup=get_main_keyboard(message.from_user.id)
        )
    except ValueError:
        await message.answer(
            f"{error_emoji} Введите корректное положительное число:",
            reply_markup=get_cancel_keyboard()
        )

@dp.message(SetupStates.waiting_for_discount_threshold)
async def process_discount(message: Message, state: FSMContext):
    ...
    try:
        ...
        db.update_settings(discount_threshold=threshold, discount_percent=percent)
        
        await state.clear()
        await message.answer(
            f"{success_emoji} Скидка настроена: {percent}% при сумме от {threshold} ₽",
            reply_markup=get_main_keyboard(message.from_user.id)
        )
    except ValueError:
        await message.answer(
            f"{error_emoji} Введите два числа через пробел (например: 5000 5):",
            reply_markup=get_cancel_keyboard()
        )

@dp.message(SetupStates.waiting_for_bonus_percent)
async def process_bonus_percent(message: Message, state: FSMContext):
    ...
    try:
        ...
        db.update_settings(referral_bonus_percent=percent)
        
        await state.clear()
        await message.answer(
            f"{success_emoji} Реферальный бонус установлен: {percent}%",
            reply_markup=get_main_keyboard(message.from_user.id)
        )
    except ValueError:
        await message.answer(
...
@dp.message(lambda message: message.text and "Правила" in message.text)
async def show_rules(message: Message):
    ...
    await message.answer(
        f"{rules_emoji} Правила магазина:\n\n{rules_text}"
    )

@dp.message(lambda message: message.text and "Доставка" in message.text)
async def show_delivery(message: Message):
    ...
    await message.answer(
        f"{delivery_emoji} Информация о доставке:\n\n{delivery_text}"
    )

@dp.message(lambda message: message.text and "Оплата" in message.text)
async def show_payment_info(message: Message):
    ...
    await message.answer(
        f"{payment_emoji} Способы оплаты:\n\n{payment_text}"
    )

@dp.message(lambda message: message.text and "Контакты" in message.text)
async def show_contacts(message: Message):
    ...
    await message.answer(
        f"{contact_emoji} Контактная информация:\n\n{contact_text}"
    )
...
async def process_quantity(callback: CallbackQuery, state: FSMContext):
    ...
    if data[1] == "custom":
        ...
        fake_data = f"product_{product_id}_paid"
...
async def create_payment(message: types.Message, product_id: int, quantity: int, user_id: int):
    ...
    if min_order > 0 and total_rub < min_order:
        await message.answer(
            f"{error_emoji} Минимальная сумма заказа: {min_order} ₽. Ваша сумма: {total_rub} ₽"
        )
        return
    
    if total_rub > max_order:
        await message.answer(
            f"{error_emoji} Максимальная сумма заказа: {max_order} ₽. Ваша сумма: {total_rub} ₽"
        )
        return
...
@dp.message(lambda message: message.text and "Добавить платный товар" in message.text)
async def add_paid_product_start(message: Message, state: FSMContext):
    if not db.is_admin(message.from_user.id):
        error_emoji = db.get_emoji(EmojiType.ERROR.value)
        await message.answer(f"{error_emoji} У вас нет доступа к этой функции.")
        return
    
    await state.update_data(product_type='paid')
    ...
    for category in categories:
        builder.row(InlineKeyboardButton(
            text=f"{category_emoji} {category['name']}",
            callback_data=f"add_paid_product_cat_{category['id']}"
        ))
    
    builder.row(InlineKeyboardButton(text="❌ Отмена", callback_data="back_to_admin"))
    
    await message.answer(
        "💰 Выберите категорию для платного товара:",
        reply_markup=builder.as_markup()
    )
    await state.set_state(AdminStates.waiting_for_paid_product_category)

@dp.callback_query(AdminStates.waiting_for_paid_product_category, F.data.startswith("add_paid_product_cat_"))
async def process_paid_product_category(callback: CallbackQuery, state: FSMContext):
    category_id = int(callback.data.replace("add_paid_product_cat_", ""))
    await state.update_data(product_category_id=category_id)
    
    await callback.message.edit_text(
        "💰 Введите название платного товара:"
    )
    await state.set_state(AdminStates.waiting_for_paid_product_name)
    await callback.answer()

@dp.message(AdminStates.waiting_for_paid_product_name)
async def process_paid_product_name(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    ...
    await message.answer(
        "💰 Введите описание платного товара:",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_paid_product_description(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
...
async def process_paid_product_price(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
...
async def process_paid_product_quantity(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    ...
    await state.set_state(AdminStates.waiting_for_paid_product_photo)

@dp.message(AdminStates.waiting_for_paid_product_photo)
async def process_paid_product_photo(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
...
async def process_paid_product_content(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    success_emoji = db.get_emoji(EmojiType.SUCCESS.value)
    ...
    await state.clear()
    await message.answer(
        f"{success_emoji} Платный товар '{data['product_name']}' успешно добавлен!\n"
        f"Цена: {data['product_price']} ₽\n"
        f"Количество: {data['product_quantity']} шт.",
        reply_markup=get_admin_panel_keyboard()
    )

# ==================== Обработчики для бесплатных товаров ====================

@dp.message(lambda message: message.text and "Добавить бесплатный" in message.text)
async def add_free_product_start(message: Message, state: FSMContext):
    if not db.is_admin(message.from_user.id):
        error_emoji = db.get_emoji(EmojiType.ERROR.value)
        await message.answer(f"{error_emoji} У вас нет доступа к этой функции.")
        return
    
    await state.update_data(product_type='free')
    ...
    for category in categories:
        builder.row(InlineKeyboardButton(
            text=f"{category_emoji} {category['name']}",
            callback_data=f"add_free_product_cat_{category['id']}"
        ))
    
    builder.row(InlineKeyboardButton(text="❌ Отмена", callback_data="back_to_admin"))
    
    await message.answer(
        "🎁 Выберите категорию для бесплатного товара:",
        reply_markup=builder.as_markup()
    )
    await state.set_state(AdminStates.waiting_for_free_product_category)

@dp.callback_query(AdminStates.waiting_for_free_product_category, F.data.startswith("add_free_product_cat_"))
async def process_free_product_category(callback: CallbackQuery, state: FSMContext):
    category_id = int(callback.data.replace("add_free_product_cat_", ""))
    await state.update_data(product_category_id=category_id)
    
    await callback.message.edit_text(
        "🎁 Введите название бесплатного товара:"
    )
    await state.set_state(AdminStates.waiting_for_free_product_name)
    await callback.answer()

@dp.message(AdminStates.waiting_for_free_product_name)
async def process_free_product_name(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    ...
    await state.update_data(product_name=message.text)
    await message.answer(
        "🎁 Введите описание бесплатного товара:",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_free_product_description(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    ...
    await state.update_data(product_description=message.text)
    await message.answer(
        "🎁 Введите количество бесплатного товара в наличии (целое число):",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_free_product_quantity(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    ...
    await state.set_state(AdminStates.waiting_for_free_product_photo)

@dp.message(AdminStates.waiting_for_free_product_photo)
async def process_free_product_photo(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    ...
    await state.update_data(product_photo=photo_file_id)
    await message.answer(
        "🎁 Введите контент бесплатного товара (ссылку, текст или файл, который получит пользователь):",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_free_product_content(message: Message, state: FSMContext):
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    success_emoji = db.get_emoji(EmojiType.SUCCESS.value)
    ...
    await state.clear()
    await message.answer(
        f"{success_emoji} Бесплатный товар '{data['product_name']}' успешно добавлен!\n"
        f"Количество: {data['product_quantity']} шт.",
        reply_markup=get_admin_panel_keyboard()
    )
...
async def process_edit_promo(message: Message, state: FSMContext):
    ...
    # Определяем, что редактируем по наличию в callback
    # В реальном приложении нужно хранить это в состоянии
    db.update_promocode(promo_id, discount_percent=value)
