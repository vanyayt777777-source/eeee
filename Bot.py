...
DEFAULT_EMOJIS = {
    EmojiType.MAIN_MENU.value: "🏠",
    EmojiType.BUY.value: "🛒",
    EmojiType.STOCK.value: "📦",
    EmojiType.PROFILE.value: "👤",
    EmojiType.ABOUT.value: "ℹ",
    EmojiType.ADMIN.value: "🔧",
    EmojiType.SETTINGS.value: "⚙",
    EmojiType.STATS.value: "📊",
    EmojiType.NEWSLETTER.value: "📨",
    EmojiType.CATEGORY.value: "📁",
    EmojiType.PRODUCT.value: "📦",
    EmojiType.PAYMENT.value: "💳",
    EmojiType.REFERRAL.value: "🔗",
    EmojiType.SUCCESS.value: "✅",
    EmojiType.ERROR.value: "❌",
    EmojiType.CART.value: "🛍",
    EmojiType.BACK.value: "◀"
}
...
class Database:
    ...
    def get_emoji(self, emoji_type: str) -> str:
        with self.get_connection() as conn:
            ...
            return DEFAULT_EMOJIS.get(emoji_type, "•")
...
def get_emoji(emoji_type: str) -> str:
    """Получает эмодзи из БД"""
    return db.get_emoji(emoji_type)
...
def get_main_keyboard(telegram_id: int) -> ReplyKeyboardMarkup:
    ...
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.BUY.value)} Купить товар"))
    builder.row(
        KeyboardButton(text=f"{get_emoji(EmojiType.STOCK.value)} Наличие товара"), 
        KeyboardButton(text=f"{get_emoji(EmojiType.PROFILE.value)} Профиль")
    )
    builder.row(
        KeyboardButton(text=f"{get_emoji(EmojiType.REFERRAL.value)} Реферальная программа"),
        KeyboardButton(text=f"{get_emoji(EmojiType.ABOUT.value)} О нас")
    )
    ...
    if not is_setup_complete:
        ...
        if settings.get('admin_id') is None:
            builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.SETTINGS.value)} Стать администратором"))
        # Если пользователь является админом и настройки не завершены
        elif is_admin:
            builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.SETTINGS.value)} Настроить бота"))
    
    # Кнопка админ-панели для админа в рабочем режиме
    if is_admin and is_setup_complete:
        builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.ADMIN.value)} Админ панель"))
    
    return builder.as_markup(resize_keyboard=True)

def get_admin_panel_keyboard() -> ReplyKeyboardMarkup:
    builder = ReplyKeyboardBuilder()
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.STATS.value)} Статистика"))
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.NEWSLETTER.value)} Рассылка"))
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.CATEGORY.value)} Добавление категорий"))
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.PRODUCT.value)} Добавление товаров"))
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.SETTINGS.value)} Управление эмодзи"))
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.BACK.value)} Назад в главное меню"))
    return builder.as_markup(resize_keyboard=True)

def get_cancel_keyboard() -> ReplyKeyboardMarkup:
    builder = ReplyKeyboardBuilder()
    builder.row(KeyboardButton(text=f"{get_emoji(EmojiType.ERROR.value)} Отмена"))
    return builder.as_markup(resize_keyboard=True)
...
def get_categories_keyboard() -> InlineKeyboardMarkup:
    ...
    for category in categories:
        builder.row(InlineKeyboardButton(
            text=f"{get_emoji(EmojiType.CATEGORY.value)} {category['name']}",
            callback_data=f"category_{category['id']}"
        ))
    
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.BACK.value)} Назад", callback_data="back_to_main"))
    return builder.as_markup()
...
def get_products_keyboard(category_id: int) -> InlineKeyboardMarkup:
    ...
    for product in products:
        ...
        builder.row(InlineKeyboardButton(
            text=f"{get_emoji(EmojiType.PRODUCT.value)} {product['name']} - {price} {currency_symbol}",
            callback_data=f"product_{product['id']}"
        ))
    
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.BACK.value)} Назад к категориям", callback_data="back_to_categories"))
    return builder.as_markup()

def get_payment_keyboard(payment_url: str, purchase_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.PAYMENT.value)} Оплатить", url=payment_url))
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.SUCCESS.value)} Проверить оплату", callback_data=f"check_payment_{purchase_id}"))
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.ERROR.value)} Отмена", callback_data="cancel_payment"))
    return builder.as_markup()

def get_back_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.BACK.value)} Назад", callback_data="back"))
    return builder.as_markup()
...
def get_emojis_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.BACK.value)} Назад", callback_data="back_to_admin"))
    return builder.as_markup()
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                payload = {
                    ...
                    "paid_btn_url": f"https://t.me/{(await bot.get_me()).username}",  # Ссылка на бота
...
async def cmd_start(message: Message):
    ...
    welcome_text = f"{get_emoji(EmojiType.MAIN_MENU.value)} Добро пожаловать в {settings.get('bot_name', 'магазин')}!\n\n"
    ...
    if referral_code:
        ...
        if referrer and referrer['telegram_id'] != user_id:
            welcome_text += f"\n\n{get_emoji(EmojiType.SUCCESS.value)} Вы перешли по реферальной ссылке!"
...
@dp.message(F.text == f"{get_emoji(EmojiType.SETTINGS.value)} Стать администратором")
async def become_admin(message: Message):
    ...
    if settings.get('admin_id') is not None:
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Администратор уже назначен.",
    ...
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Вы назначены администратором!\n\n"
        f"Теперь нажмите кнопку 'Настроить бота' для продолжения настройки.",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(F.text == f"{get_emoji(EmojiType.SETTINGS.value)} Настроить бота")
async def setup_bot(message: Message, state: FSMContext):
    """Начало настройки бота"""
    if not db.is_admin(message.from_user.id):
        await message.answer(f"{get_emoji(EmojiType.ERROR.value)} У вас нет прав для настройки бота.")
        return
...
async def setup_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if not db.is_admin(callback.from_user.id):
        await callback.answer(f"{get_emoji(EmojiType.ERROR.value)} У вас нет прав для настройки бота.")
        return
    ...
    if action == "name":
        ...
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.SUCCESS.value)} Настройка завершена! Бот запущен в рабочем режиме."
        )
...
async def process_bot_name(message: Message, state: FSMContext):
    ...
    if message.text == f"{get_emoji(EmojiType.ERROR.value)} Отмена":
        await state.clear()
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Настройка отменена.",
            reply_markup=get_main_keyboard(message.from_user.id)
        )
        return
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Имя бота изменено на: {message.text}",
        reply_markup=get_main_keyboard(message.from_user.id)
    )
...
async def process_welcome_message(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Приветственное сообщение изменено!",
        reply_markup=get_main_keyboard(message.from_user.id)
    )
...
async def process_currency(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text(
        f"{get_emoji(EmojiType.SUCCESS.value)} Валюта изменена на: {'Рубли (₽)' if currency == 'RUB' else 'Доллары ($)'}"
    )
    await callback.answer()
...
async def process_admin_id(message: Message, state: FSMContext):
    ...
    if not message.text.isdigit():
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Ошибка: admin ID должен быть числом. Попробуйте снова:",
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Admin ID изменен на: {admin_id}",
        reply_markup=get_main_keyboard(message.from_user.id)
    )
...
async def process_crypto_token(message: Message, state: FSMContext):
    ...
    if len(message.text) < 10:
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Ошибка: неверный формат токена. Попробуйте снова:",
            reply_markup=get_cancel_keyboard()
        )
        return
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Crypto Bot API токен сохранен.",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

@dp.message(F.text == f"{get_emoji(EmojiType.REFERRAL.value)} Реферальная программа")
async def referral_program(message: Message):
    ...
    if not user:
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Профиль не найден."
        )
        return
    ...
    # Формируем сообщение
    message_text = (
        f"{get_emoji(EmojiType.REFERRAL.value)} *Реферальная программа*\n\n"
...
@dp.message(F.text == f"{get_emoji(EmojiType.BUY.value)} Купить товар")
async def buy_product(message: Message):
    ...
    if not settings.get('is_setup_complete'):
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Магазин еще не настроен. Дождитесь завершения настройки администратором."
        )
        return
    ...
    if not categories:
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} В магазине пока нет товаров."
        )
        return
    
    await message.answer(
        f"{get_emoji(EmojiType.CATEGORY.value)} Выберите категорию:",
...
async def show_category_products(callback: CallbackQuery):
    ...
    if not products:
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.ERROR.value)} В категории '{category['name']}' пока нет товаров.",
            reply_markup=get_back_keyboard()
        )
        await callback.answer()
        return
    ...
    await callback.message.edit_text(
        f"{get_emoji(EmojiType.CATEGORY.value)} *Категория: {category['name']}*\n\n"
        f"{category['description']}\n\n"
        f"{get_emoji(EmojiType.PRODUCT.value)} Выберите товар:",
        parse_mode="Markdown",
...
async def show_product_details(callback: CallbackQuery):
    ...
    if not product:
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.ERROR.value)} Товар не найден.",
            reply_markup=get_back_keyboard()
        )
        await callback.answer()
        return
    ...
    message_text = (
        f"{get_emoji(EmojiType.PRODUCT.value)} *{product['name']}*\n\n"
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.PAYMENT.value)} Купить", callback_data=f"buy_{product_id}"))
    builder.row(InlineKeyboardButton(text=f"{get_emoji(EmojiType.BACK.value)} Назад", callback_data=f"category_{product['category_id']}"))
...
async def process_purchase(callback: CallbackQuery):
    ...
    if not settings.get('crypto_token'):
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.ERROR.value)} Ошибка: платежная система не настроена. Обратитесь к администратору.",
            reply_markup=get_back_keyboard()
        )
        await callback.answer()
        return
    ...
    if not invoice:
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.ERROR.value)} Ошибка при создании счета. Попробуйте позже.",
            reply_markup=get_back_keyboard()
        )
        await callback.answer()
        return
    ...
    await callback.message.edit_text(
        f"{get_emoji(EmojiType.PAYMENT.value)} *Счет на оплату:*\n\n"
...
async def check_payment(callback: CallbackQuery):
    ...
    if not purchase:
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.ERROR.value)} Платеж не найден.",
            reply_markup=get_back_keyboard()
        )
        await callback.answer()
        return
    ...
    if status == 'paid':
        ...
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.PAYMENT.value)} Счет ожидает оплаты. После оплаты нажмите кнопку проверки.",
        ...
        await callback.message.edit_text(
            f"{get_emoji(EmojiType.ERROR.value)} Платеж не найден или истек. Попробуйте создать новый заказ.",
            reply_markup=get_back_keyboard()
        )
...
async def deliver_product(message: types.Message, purchase: Dict):
    ...
    if not product:
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Ошибка: товар не найден. Обратитесь к администратору."
        )
        return
    
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} *Оплата получена!*\n\n"
        f"{get_emoji(EmojiType.PRODUCT.value)} *Ваш товар:*\n\n{product['content']}",
        parse_mode="Markdown"
    )
...
async def cancel_payment(callback: CallbackQuery):
    """Отмена платежа"""
    await callback.message.edit_text(
        f"{get_emoji(EmojiType.ERROR.value)} Платеж отменен."
    )
...
@dp.message(F.text == f"{get_emoji(EmojiType.STOCK.value)} Наличие товара")
async def check_stock(message: Message):
    ...
    message_text = f"{get_emoji(EmojiType.STOCK.value)} *Наличие товаров:*\n\n"
    
    for category in categories:
        category_products = db.get_products_by_category(category['id'])
        if category_products:
            message_text += f"{get_emoji(EmojiType.CATEGORY.value)} *{category['name']}:*\n"
...
@dp.message(F.text == f"{get_emoji(EmojiType.PROFILE.value)} Профиль")
async def show_profile(message: Message):
    ...
    if not user:
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Профиль не найден. Начните с команды /start"
        )
        return
    ...
    profile_text = (
        f"{get_emoji(EmojiType.PROFILE.value)} *Ваш профиль:*\n\n"
...
@dp.message(F.text == f"{get_emoji(EmojiType.ABOUT.value)} О нас")
async def about(message: Message):
    ...
    await message.answer(
        f"{get_emoji(EmojiType.ABOUT.value)} *О магазине '{settings.get('bot_name')}':*\n\n"
...
@dp.message(F.text == f"{get_emoji(EmojiType.ADMIN.value)} Админ панель")
async def admin_panel(message: Message):
    """Вход в админ-панель"""
    if not db.is_admin(message.from_user.id):
        await message.answer(f"{get_emoji(EmojiType.ERROR.value)} У вас нет доступа к админ-панели.")
        return
    
    await message.answer(
        f"{get_emoji(EmojiType.ADMIN.value)} *Админ-панель*\n\nВыберите действие:",
        parse_mode="Markdown",
        reply_markup=get_admin_panel_keyboard()
    )

@dp.message(F.text == f"{get_emoji(EmojiType.STATS.value)} Статистика")
async def show_statistics(message: Message):
    """Показ статистики"""
    if not db.is_admin(message.from_user.id):
        await message.answer(f"{get_emoji(EmojiType.ERROR.value)} У вас нет доступа к этой функции.")
        return
    
    stats = db.get_statistics()
    
    await message.answer(
        f"{get_emoji(EmojiType.STATS.value)} *Статистика магазина:*\n\n"
...
@dp.message(F.text == f"{get_emoji(EmojiType.NEWSLETTER.value)} Рассылка")
async def start_newsletter(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"{get_emoji(EmojiType.NEWSLETTER.value)} Введите текст для рассылки всем пользователям:",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_newsletter(message: Message, state: FSMContext):
    ...
    if message.text == f"{get_emoji(EmojiType.ERROR.value)} Отмена":
        await state.clear()
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Рассылка отменена.",
            reply_markup=get_admin_panel_keyboard()
        )
        return
    ...
    for user in users:
        try:
            await bot.send_message(
                ...
                text=f"{get_emoji(EmojiType.NEWSLETTER.value)} *Рассылка от администратора:*\n\n{message.text}",
                parse_mode="Markdown"
            )
            success_count += 1
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} *Рассылка завершена!*\n"
...
@dp.message(F.text == f"{get_emoji(EmojiType.CATEGORY.value)} Добавление категорий")
async def add_category_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"{get_emoji(EmojiType.CATEGORY.value)} Введите название категории:",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_category_name(message: Message, state: FSMContext):
    ...
    if message.text == f"{get_emoji(EmojiType.ERROR.value)} Отмена":
        await state.clear()
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Добавление категории отменено.",
            reply_markup=get_admin_panel_keyboard()
        )
        return
    ...
    await message.answer(
        f"{get_emoji(EmojiType.CATEGORY.value)} Введите описание категории:",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_category_description(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Категория '{category_name}' успешно добавлена!",
        reply_markup=get_admin_panel_keyboard()
    )

@dp.message(F.text == f"{get_emoji(EmojiType.PRODUCT.value)} Добавление товаров")
async def add_product_start(message: Message, state: FSMContext):
    ...
    if not categories:
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Сначала добавьте хотя бы одну категорию!",
            reply_markup=get_admin_panel_keyboard()
        )
        return
    ...
    await message.answer(
        f"{get_emoji(EmojiType.PRODUCT.value)} Выберите категорию для товара:",
        reply_markup=builder.as_markup()
    )
...
async def process_product_category(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"{get_emoji(EmojiType.PRODUCT.value)} Введите название товара:"
    )
...
async def process_product_name(message: Message, state: FSMContext):
    ...
    if message.text == f"{get_emoji(EmojiType.ERROR.value)} Отмена":
        await state.clear()
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Добавление товара отменено.",
            reply_markup=get_admin_panel_keyboard()
        )
        return
    ...
    await message.answer(
        f"{get_emoji(EmojiType.PRODUCT.value)} Введите описание товара:",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_product_description(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"{get_emoji(EmojiType.PRODUCT.value)} Введите цену товара в рублях (число, например: 1000):",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_product_price(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Ошибка: введите корректное положительное число.",
            reply_markup=get_cancel_keyboard()
        )
        return
    ...
    await message.answer(
        f"{get_emoji(EmojiType.PRODUCT.value)} Отправьте фото товара (необязательно, для пропуска отправьте 'пропустить'):\n"
        "Или отправьте фото:",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_product_photo(message: Message, state: FSMContext):
    ...
    if message.text and message.text.lower() == 'пропустить':
        ...
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Пожалуйста, отправьте фото или напишите 'пропустить':",
            reply_markup=get_cancel_keyboard()
        )
        return
    ...
    await message.answer(
        f"{get_emoji(EmojiType.PRODUCT.value)} Введите контент товара (ссылку, текст или файл, который получит пользователь после оплаты):",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_product_content(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Товар '{data['product_name']}' успешно добавлен!",
        reply_markup=get_admin_panel_keyboard()
    )

@dp.message(F.text == f"{get_emoji(EmojiType.SETTINGS.value)} Управление эмодзи")
async def manage_emojis(message: Message, state: FSMContext):
    ...
    text = f"{get_emoji(EmojiType.SETTINGS.value)} *Управление эмодзи*\n\n"
...
async def process_emoji_id(message: Message, state: FSMContext):
    ...
    if message.text == f"{get_emoji(EmojiType.ERROR.value)} Отмена":
        await state.clear()
        await message.answer(
            f"{get_emoji(EmojiType.ERROR.value)} Редактирование отменено.",
            reply_markup=get_admin_panel_keyboard()
        )
        return
    ...
    await state.clear()
    await message.answer(
        f"{get_emoji(EmojiType.SUCCESS.value)} Эмодзи для '{emoji_type}' обновлен!",
        reply_markup=get_admin_panel_keyboard()
    )

@dp.message(F.text == f"{get_emoji(EmojiType.BACK.value)} Назад в главное меню")
...
async def back_callback(callback: CallbackQuery):
    ...
    await callback.message.delete()
    await callback.message.answer(
        f"{get_emoji(EmojiType.BACK.value)} Действие отменено."
    )
    await callback.answer()
...
async def handle_unknown(message: Message):
    ...
    await message.answer(
        f"{get_emoji(EmojiType.ERROR.value)} Я не понимаю эту команду. Пожалуйста, используйте кнопки меню.",
        reply_markup=get_main_keyboard(message.from_user.id)
    )
