...
class Database:
        ...
        with self.get_connection() as conn:
            ...
            if row:
                if row['is_premium'] and row['premium_emoji_id']:
                    # Премиум эмодзи в правильном формате
    ...
    def add_product(self, category_id: int, name: str, description: str, 
                   price_rub: float, quantity: int, content: str, photo_file_id: Optional[str] = None) -> int:
        price_usd = round(price_rub / USDT_TO_RUB, 2)
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO products (category_id, name, description, price_rub, price_usd, quantity, content, photo_file_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (category_id, name, description, price_rub, price_usd, quantity, content, photo_file_id))
            return cursor.lastrowid
    ...
    def create_purchase(self, user_id: int, product_id: int, quantity: int, 
                       amount_rub: float, amount_usd: float, crypto_payment_id: str) -> int:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                INSERT INTO purchases (user_id, product_id, quantity, amount_rub, amount_usd, crypto_payment_id, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, product_id, quantity, amount_rub, amount_usd, crypto_payment_id, 'pending'))
            return cursor.lastrowid
...
def get_quantity_keyboard(product_id: int, max_quantity: int) -> InlineKeyboardMarkup:
    ...
    # Кнопка отмены
    back_emoji = db.get_emoji(EmojiType.BACK.value)
    builder.row(InlineKeyboardButton(text=f"{back_emoji} Назад", callback_data=f"category_{product_id}"))
...
async def process_about_message(message: Message, state: FSMContext):
    """Обработка ввода сообщения 'О нас'"""
    error_emoji = db.get_emoji(EmojiType.ERROR.value)
    success_emoji = db.get_emoji(EmojiType.SUCCESS.value)
...
async def show_product_details(callback: CallbackQuery):
    """Показ деталей товара и предложение выбора количества"""
    ...
    await callback.message.edit_text(
        ...
        parse_mode="Markdown",
        reply_markup=get_quantity_keyboard(product_id, product['quantity'])
    )
    await callback.answer()
...
async def process_quantity(callback: CallbackQuery, state: FSMContext):
    ...
    if data[1] == "custom":
        ...
        quantity = int(data[2])
        await create_payment(callback.message, product_id, quantity)
...
async def process_custom_quantity(message: Message, state: FSMContext):
    ...
    try:
        ...
        await state.clear()
        await create_payment(message, product_id, quantity)
...
async def create_payment(message: types.Message, product_id: int, quantity: int):
    """Создание платежа для товара"""
    product = db.get_product(product_id)
    settings = db.get_settings()
    ...
    # Проверяем наличие товара и резервируем
...
async def about(message: Message):
    ...
    # Используем форматирование с поддержкой премиум эмодзи
    formatted_about = db.format_emoji(EmojiType.ABOUT.value)
    
    about_text = settings.get('about_message', 'Информация о магазине')
    
    await message.answer(
        f"{formatted_about} *О магазине '{settings.get('bot_name')}':*\n\n"
        f"{about_text}\n\n"
...
async def process_product_price(message: Message, state: FSMContext):
    ...
    product_emoji = db.get_emoji(EmojiType.PRODUCT.value)
    await message.answer(
        f"{product_emoji} Введите количество товара в наличии (целое число):",
        reply_markup=get_cancel_keyboard()
    )
...
async def process_emoji_id(message: Message, state: FSMContext):
    ...
    if '|' in message.text:
        # Премиум эмодзи в формате "⭐|123456789"
